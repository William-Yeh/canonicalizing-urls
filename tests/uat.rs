//! User Acceptance Tests — end-to-end canonicalization driven by BEFORE→AFTER
//! tables. Each row is `(description, input, expected)`. The table IS the
//! specification: readable by humans, runnable by `cargo test`.
//!
//! Ported 1:1 from the Python `tests/test_uat.py` (29 OFFLINE + 2 ONLINE rows).

use canonicalize::engine::canonicalize;
use canonicalize::rules::rules;

fn no_net(_url: &str) -> String {
    unreachable!("offline UAT row must not hit the network")
}

// ---------------------------------------------------------------------------
// Offline UAT table: (description, before, after)
// ---------------------------------------------------------------------------
const OFFLINE: &[(&str, &str, &str)] = &[
    ...
    ("LinkedIn learning-login: unwrap redirect, strip account/trk",
     "https://www.linkedin.com/learning-login/share?account=352396234&forceAccount=false&redirect=https%3A%2F%2Fwww.linkedin.com%2Flearning%2Fcourse-name",
...
// ---------------------------------------------------------------------------
// Online UAT table: (description, before, mocked_resolved_url, after)
// ---------------------------------------------------------------------------
const ONLINE: &[(&str, &str, &str, &str)] = &[
    (
...
#[test]
fn offline_uat_table() {
    let rules = rules();
    for (desc, before, after) in OFFLINE {
        let got = canonicalize(before, &rules, false, &no_net);
        assert_eq!(&got, after, "OFFLINE failed: {desc}\n  before: {before}");
    }
}
...
fn online_uat_table() {
    let rules = rules();
    for (desc, before, resolved, after) in ONLINE {
        let resolver = move |_url: &str| resolved.to_string();
        let got = canonicalize(before, &rules, true, &resolver);
        assert_eq!(&got, after, "ONLINE failed: {desc}\n  before: {before}");
    }
}
